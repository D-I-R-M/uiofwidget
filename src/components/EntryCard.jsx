...
function activityColor(id) { return ACTIVITY_COLORS[id] || '#555' }
function activityLabel(id) { return ACTIVITY_LABELS[id] || id.split('.').pop() }
  ...
  return (
    <Link to={`/entries/${entry.uid}`} className={styles.card} style={style}>
      <div className={styles.top}>
        ...
        {entry.keep && <span className={styles.star}>★</span>}
      </div>
